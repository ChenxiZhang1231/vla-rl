...
def process_timestamp_folder(date_folder, timestamp_folder, raw_folder, save_path, urdf_logger):
...
def process_date_folder(date_folder, raw_folder, save_path, urdf_logger, max_workers=4):
    date_path = raw_folder / date_folder
    ...
    with ThreadPoolExecutor(max_workers=max_workers) as executor:
        ...
        for timestamp_folder in timestamp_folders:
            futures.append(executor.submit(
                process_timestamp_folder, date_folder, timestamp_folder, raw_folder, save_path, urdf_logger
            ))
        for future in tqdm(as_completed(futures), total=len(futures), desc=f"Processing timestamp in {date_folder}", leave=False):
            future.result()
...
def main():
    ...
    parser.add_argument("--save_path", default='data/droid_processed_data/', type=Path)
    ...
    with ThreadPoolExecutor(max_workers=1) as executor:
        ...
        for date_folder in date_folders:
            futures.append(executor.submit(
                process_date_folder, date_folder, raw_folder, save_path, urdf_logger, args.num_threads
            ))

        for future in tqdm(as_completed(futures), total=len(futures), desc="Processing date folders"):
            future.result()
