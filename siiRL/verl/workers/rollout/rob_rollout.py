...
from .video_emb import VideoEmbeddingModel
...
class RobHFRollout(BaseRollout):
    ...
    def _generate_chunk_rollout(self, prompts):
        ...
        assert np.all(task_suite_name == self.config.task_suite_name), \
        ...
        for k in task_records:
            tasks_for_embedding.append((
                ...
                self.config.experiment_name + "rollout4embedding",
                global_steps,
                k['complete']
            ))
    ...
    def _obs_to_input(self, obs):
        from verl.utils.libero_utils import get_libero_image, get_libero_wrist_image, quat2axisangle
