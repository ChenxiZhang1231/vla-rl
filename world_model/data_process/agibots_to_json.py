...
valid_cameras = ['head_color.mp4', 'hand_left_color.mp4', 'hand_right_color.mp4']
name_to_e_params = {'head_color.mp4': 'head_extrinsic_params_aligned.json',
                  'hand_left_color.mp4': 'hand_left_extrinsic_params_aligned.json',
                  'hand_right_color.mp4': 'hand_right_extrinsic_params_aligned.json',}
name_to_e_params_backpack = {'head_color.mp4': 'head_extrinsic_params.json',
                  'hand_left_color.mp4': 'hand_left_extrinsic_params.json',
                  'hand_right_color.mp4': 'hand_right_extrinsic_params.json',}
name_to_k_params = {'head_color.mp4': 'head_intrinsic_params.json',
                  'hand_left_color.mp4': 'hand_left_intrinsic_params.json',
                  'hand_right_color.mp4': 'hand_right_intrinsic_params.json',}
...
def process_video(video_path: Path, out_root: Path, fps: int, clip_len: int, stride: int,
                  ...
                  end_pos_orig: np.ndarray = None,     # (N, 2, 3)
                  end_quat_orig: np.ndarray = None,    # (N, 2, 4)
                  eff_pos_orig: np.ndarray = None,     # (N, 2, 2)
    ...
    pos_res = end_pos_orig[idx_map]                   # (F,2,3)
    quat_res = end_quat_orig[idx_map]                 # (F,2,4)
    eff_res = eff_pos_orig[idx_map]                  # (F,2,2)
    ...
    while f0 + clip_len <= total_frames_est:
        ...
        np.savez_compressed(npz_path,
                            extrinsics=e_clip,
                            intrinsics=k_params,
                            end_position=pos_clip,
                            end_orientation=quat_clip,
                            effector_position=eff_clip,
                            start_frame=f0,
                            clip_len=clip_len,
                            fps=fps)
...
def process_video_task(task: Dict[str, Any]) -> Dict[str, Any]:
    ...
    try:
        _, clip_infos = process_video(
            ...
            end_pos_orig=task["end_position"],
            end_quat_orig=task["end_orientation"],
            eff_pos_orig=task["effector_position"],
        )
...
def main():
    ...
    parser.add_argument("--dataset_dir", type=Path, default='/inspire/hdd/global_public/public_datas/AgiBotWorld-Beta', help="the dataset root")
    parser.add_argument("--output_dir", type=Path, default='/inspire/ssd/project/robotsimulation/public/data/AgiBotWorld-Beta-action-processed', help="processed output path")
    ...
    parser.add_argument("--jobs", type=int, default=os.cpu_count() or 4, help="parallel workers (number of concurrent videos)")
    args = parser.parse_args()

    ensure_dir(args.output_dir)

    tasks: List[Dict[str, Any]] = []
    obs_dir = args.dataset_dir / 'observations'
    cam_dir = args.dataset_dir / 'parameters'
    pro_dir = args.dataset_dir / 'proprio_stats'
    observations = sorted(os.listdir(obs_dir))
    for observation in tqdm(observations):
        task_path = args.dataset_dir / 'task_info' / f'task_{observation}.json'
        with open(task_path, "r", encoding="utf-8") as f:
            task_info = json.load(f)
            task_name = task_info[0]['task_name']
            description = task_name + '.'

        sessions = sorted(os.listdir(obs_dir / observation))[:20]
        for session in sessions:
            videos_dir = obs_dir / observation / session / 'videos'
            camera_param_dir = cam_dir / observation / session / 'parameters'/ 'camera'
            proprio_stats_path = pro_dir / observation / session / 'proprio_stats.h5'
            if not videos_dir.exists():
                continue
            videos = sorted(os.listdir(videos_dir))
            with h5py.File(proprio_stats_path, "r") as f:
                proprio = h5_to_dict(f)
                try:
                    end_orientation = proprio['state']['end']['orientation'] #(N, 2, 4)
                    end_position = proprio['state']['end']['position'] #(N, 2, 3)
                    effector_position = proprio['state']['effector']['position'] #(N, 2, 2)
                except:
                    continue

            for video in videos:
                if video in valid_cameras:
                    vp = videos_dir / video
                    e_params_path = camera_param_dir / name_to_e_params[video]
                    if not os.path.exists(e_params_path):
                        e_params_path = camera_param_dir / name_to_e_params_backpack[video]
                        if not os.path.exists(e_params_path):
                            continue
                    k_params_path = camera_param_dir / name_to_k_params[video]
                    if not os.path.exists(k_params_path):
                        continue
                    with open(e_params_path, "r", encoding="utf-8") as f:
                        e_params = json.load(f)
                        e_params = preprocess_e_params(e_params) # (N, 4, 4)
                    with open(k_params_path, "r", encoding="utf-8") as f:
                        k_params = json.load(f)
                        k_params = preprocess_k_params(k_params) # (3, 3)
                    tasks.append(dict(
                        video_path=vp,
