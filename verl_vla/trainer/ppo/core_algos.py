...
def compute_grpo_outcome_advantage_smolvla_raw(token_level_rewards: torch.Tensor,
                                            eos_mask: torch.Tensor,
                                            index: torch.Tensor,
                                            epsilon: float = 1e-6):
    """
    ...
    Returns:
        ...
        Returns: `(torch.Tensor)`
            shape: (bs, response_length)
    """
    response_length = token_level_rewards.shape[-2]
    scores = token_level_rewards.sum(dim=(-1, -2))
...
def compute_rloo_returns(data:verl_vla.DataProto, eos_mask:torch.Tensor,n_samples,config):
...
def kl_penalty(
    ...
    """
    if kl_penalty in ["kl", "dwc_pg", 'kl_kwise', 'kl_ffp', 'kwise_ffp']:
