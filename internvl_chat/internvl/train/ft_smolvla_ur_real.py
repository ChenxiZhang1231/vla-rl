...
from internvl.model.smolvla import SmolVLAPolicyUR, SmolVLAConfig
...
class LeRobotDataset(Dataset):
    ...
    def __getitem__lerobot(self, idx) -> dict:
    ...
    def __getitem__(self, idx) -> dict:
        ...
        while True:
            ...
            try:
                ret_dcit = {}
                item = self.data[idx]
                if len(item['observation.images.front']) == 1:
                    image = Image.open(os.path.join(self.root, item['observation.images.front'][0])).convert('RGB')
                img_tensor = TF.to_tensor(image) 
                ret_dcit['observation.images.front'] = img_tensor
                
                
                for key, values in item.items():
                    if key not in ["task", "observation.images.front"]:
...
def main():
    ...
    model = SmolVLAPolicyUR.from_pretrained(**kwargs)
