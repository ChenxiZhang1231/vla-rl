...
def concat_pad_data_collator_smovla(features, max_item_length=None, pad_id=0):

    features = [feat['batch'] for feat in features]
    first = features[0]
    batch = {}
    ...
    for k, v in first.items():
        ...
        if k in ('timestamp', 'frame_index', 'episode_index', 'index', 'task_index'):
            if isinstance(v, torch.Tensor):
                batch[k] = torch.concat([f[k].unsqueeze(0) for f in features])
            elif isinstance(v, np.ndarray):
                batch[k] = torch.concat(np.stack([f[k] for f in features]))
            else:
                batch[k] = torch.concat([[f[k]] for f in features])
